...
def save_dis_sim(kgfile, sim_file):
    ...
    gene_ids = pd.read_csv(
        "/Users/alejandroadriaquelozano/Documents/Internships/MacsBio/post-covid-kg/KG/disgenet/genes_disgenet.csv"
    )
    diseases_ids = pd.read_csv(
        "/Users/alejandroadriaquelozano/Documents/Internships/MacsBio/post-covid-kg/KG/disgenet/diseases_disgenet.csv"
    )
    gene_diseases = pd.read_csv(
        "/Users/alejandroadriaquelozano/Documents/Internships/MacsBio/post-covid-kg/KG/disgenet/gene-disease_disgenet.csv"
    )
    ...
    for disease1, genes1 in disease_dict.items():
        for disease2, genes2 in disease_dict.items():
            if disease1 != disease2 and "nan" not in genes1 and "nan" not in genes2:
    ...
    for key, value in disease_similarity_matrix.items():
        for key2, value2 in value.items():
            if value2 > 0.4:
                data.append([key, key2, 2, value2, count])
                count = count + 1
