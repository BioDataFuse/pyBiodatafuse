...
def get_compounds(kegg_df):
    """Get compounds from the KEGG dataframe and update it.

    :param kegg_df: DataFrame generated by the 'get_pathways' function.
    :returns: Updated DataFrame with 'KEGG_compounds' column instead of 'KEGG_pathways'.
    """
    compound_data = {}
    queried_identifiers = set()

    kegg_df['KEGG_compounds'] = [[] for _ in range(len(kegg_df))]

    # Iterate over each row in the DataFrame to process KEGG_pathways
    for idx, row in kegg_df.iterrows():
        compounds = []
        for pathway in row['KEGG_pathways']:
            for compound in pathway['compounds']:
                kegg_id = compound['KEGG_identifier']
                if kegg_id and kegg_id not in queried_identifiers:

                    results = requests.get(f"{KEGG_ENDPOINT}/get/{kegg_id}")
                    compound_name = None
                    ...
                    compound_data[kegg_id] = {
                        "KEGG_identifier": kegg_id,
                        "name": compound_name,
                    }
                    queried_identifiers.add(kegg_id) 

                if kegg_id in compound_data:
                    compounds.append(compound_data[kegg_id])

        kegg_df.at[idx, 'KEGG_compounds'] = compounds

    kegg_df = kegg_df.drop(columns=['KEGG_pathways'])
