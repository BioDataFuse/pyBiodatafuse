...
def run_gene_annotators(
    bridgdb_df, bridgdb_metadata, selected_annotators
):
    combined_df = []
    ...
    for annotator_name in tqdm(selected_annotators):
        if annotator_name == "bgee":
            data_df, metadata = annotator_name.get_gene_expression(
                ...
                anatomical_entities=pd.DataFrame()  # TODO
            )
...
def run_annotator(
    ...
    assert isinstance(data_input, pd.DataFrame), "data_input parameter must be a pandas DataFrame. Please check your input."

    assert input_specie in ["Human"], "specie_name parameter must be one of the following: 'Human'. Please check your input."
