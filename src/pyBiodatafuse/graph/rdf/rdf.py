    ...
    def process_row(self, row: pd.Series, i: int, datasources: pd.DataFrame) -> None:
        ...
        if gene:
            try:
                ...
                logger.warning(f"Failed to process PPI data for gene node: {e}")

            try:
                protein_nodes = list(self.objects(gene_node, URIRef(Cons.PREDICATES["translation_of"])))
            except Exception as e:
                logger.warning(f"Failed to retrieve protein nodes for gene node: {e}")
                protein_nodes = []

            try:
                self.process_disease_data(disease_data, id_number, source_idx, gene_node)
            except Exception as e:
                logger.warning(f"Failed to process disease data for gene node: {e}")

            try:
                self.process_expression_data(expression_data, id_number, source_idx, gene_node)
            except Exception as e:
                logger.warning(f"Failed to process expression data for gene node: {e}")

            try:
                self.process_pathways(row, gene_node, protein_nodes)
            except Exception as e:
                logger.warning(f"Failed to process pathways for gene node: {e}")

            try:
                self.process_processes_data(processes_data, gene_node)
            except Exception as e:
                logger.warning(f"Failed to process processes data for gene node: {e}")

            try:
                self.process_compound_data(compound_data, gene_node)
            except Exception as e:
                logger.warning(f"Failed to process compound data for gene node: {e}")
            ...
            try:
                ...
                logger.warning(f"Failed to process literature data for gene node: {e}")

            try:
                self.process_transporter_inhibitor_data(gene_node, transporter_inhibitor_data)
            except Exception as e:
                logger.warning(f"Failed to process transporter inhibitor data for gene node: {e}")

            if self.include_variants:
                try:
                    self.process_protein_variants(protein_nodes)
                except Exception as e:
                    logger.warning(f"Failed to process protein variants for gene node: {e}")

            try:
                self.process_aop_data(aop_data, gene_node, None)
            except Exception as e:
                logger.warning(f"Failed to process AOP data for gene node: {e}")

            try:
                self.process_molecular_pathway(pathways_data, gene_node, id_number)
            except Exception as e:
                logger.warning(f"Failed to process molecular pathway data for gene node: {e}")

        if compound:
            try:
                self.process_pathways(row, compound_node, protein_nodes=[])
            except Exception as e:
                logger.warning(f"Failed to process pathways for compound node: {e}")

            try:
                self.process_inhibitor_transporter_data(compound_node, inhibitor_transporter_data)
            except Exception as e:
                logger.warning(f"Failed to process inhibitor transporter data for compound node: {e}")

            try:
                self.process_aop_data(aop_data, None, compound_node)
            except Exception as e:
                logger.warning(f"Failed to process AOP data for compound node: {e}")

            try:
                self.process_molecular_pathway(pathways_data, compound_node, id_number)
            except Exception as e:
                logger.warning(f"Failed to process molecular pathway data for compound node: {e}")
