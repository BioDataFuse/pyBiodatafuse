...
def main():
    ...
    print("Node with most diseases associated: {} with {} disease associations known".format(node_max, max_out_degree))
    ...
    gene_disease_edges = ((u, v) for u, v, d in g.edges(data=True) if d['label'] == 'associated_with')
    ...
    non_zero_predictions = [(gene, disease, predicted_score) for (gene, disease, predicted_score) in predictions if predicted_score > 0
