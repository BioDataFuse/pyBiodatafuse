...
os.makedirs("data/dda_files", exist_ok=True)
...
def generate_files(kg_data):
    ...
    edges_filtered = kg_data[
        kg_data["type"].isin(["activates", "inhibits", "associated_with", "interacts_with"])
    ].copy()
    ...
    output_graph.to_csv("data/graph.txt", sep="\t", index=False, header=False)
    ...
    drug_hierarchy.to_csv("data/hierarchy.csv", sep=",", index=False)
    ...
    for i in range(1, 11):
        ...
        dda_complete_df.to_csv("data/dda_files/dda" + str(i) + ".tsv", sep="\t", index=False)
...
def generate_drug_hierarchy(drug_df) -> pd.DataFrame:
    ...
    for _, row in drug_df.iterrows():
        ...
        for atc_classification in atc_classifications:
            ...
            drug_hierarchy_df.loc[len(drug_hierarchy_df) + 1] = {
                "child": drugID,
                "parent": atc_classification_list[0],
            }
