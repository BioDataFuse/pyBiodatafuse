...
def process_drugs(
    drugs_to_process, query_disease, embeddingf, model_list, map_name_dict, candidates_count
):
    ...
    for drug in drugs_to_process:
        prob_sum = 0
        for i in range(0, 10):
            prob_sum += model_list[i].predict_proba(
                [embedding_dict[drug] - embedding_dict[query_disease]]
            )[:, 1]
